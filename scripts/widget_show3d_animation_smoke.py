...
DEFAULT_CAITLYN_DIR = Path.home() / "data" / "collaboratorC" / "20260221_show3d"
...
def _load_caitlyn_stack(data_root: Path, *, crop_size: int) -> tuple[np.ndarray, list[str], dict[str, Any]]:
    ...
    return np.stack(frames, axis=0), labels, {
        "kind": "local collaboratorC Show3D PNG/TIFF time series",
        "root": str(data_root),
...
def main() -> int:
    ...
    parser.add_argument("--source", choices=["auto", "collaboratorC", "tutorial", "synthetic"], default="auto")
    ...
    for quality in args.qualities:
        path = artifact_dir / f"show3d-collaboratorC-timeseries-{quality}.gif"
