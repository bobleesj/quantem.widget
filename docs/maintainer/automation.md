...
- `showfolder-live/index.html` and `showfolder-live/report.json`: generated by
  ...
  The report also displays WebP thumbnail previews for human review and 4D-STEM
  master QC rows from header-only HDF5 inspection.
...
This script creates tiny generated image files and tiny generated HDF5
`*_master.h5` files, opens ShowFolder, activates selected Show2D/Show3D and
...
writes a dashboard row labeled `ShowFolder live-folder smoke`, WebP thumbnail
