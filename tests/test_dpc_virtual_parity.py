"""End-to-end parity for public GPU detector and DPC domains.
...
def test_dpc_phase_matches_quantem_live(gold):
    from quantem.gpu.dpc import run
...
def test_virtual_matches_manual_masked_sum(gold):
    ...
    from quantem.gpu.detector import auto_probe, detector_mask, mean_dp, virtual

    mean_pattern = np.asarray(mean_dp(gold), dtype=np.float32)
    center, r = auto_probe(mean_pattern)
    for mode in ("BF", "ABF", "ADF", "HAADF", "DF"):
        ...
        mask = detector_mask(center, *bands[mode], mean_pattern.shape)
...
def test_core_dataset4dstem_roundtrip(gold):
    ...
    from quantem.gpu.detector import adf, bf
    from quantem.gpu.dpc import center_of_mass, run
    ...
    ref = run(gold)
    result = run(ds)
    np.testing.assert_array_equal(result.phase, ref.phase)
    row, col = center_of_mass(ds)
    np.testing.assert_array_equal(col, ref.com_col)
    np.testing.assert_array_equal(row, ref.com_row)
...
def test_uint8_browse_parity(gold):
    ...
    if int(gold.max()) <= 255:  # lossless regime → virtual images bit-identical
        ...
        u8_row, u8_col = center_of_mass(u8)
        u16_row, u16_col = center_of_mass(gold)
        np.testing.assert_array_equal(u8_row, u16_row)
        np.testing.assert_array_equal(u8_col, u16_col)
    else:  # clipped: BF/ADF still faithful for screening (normalized rmse small)
        def nrmse(a, b):
            a = (a - a.min()) / (np.ptp(a) + 1e-9)
            b = (b - b.min()) / (np.ptp(b) + 1e-9)
